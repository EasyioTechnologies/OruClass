...
describe("jwt auth tokens", () => {
  it("round-trips an access token preserving userId + email", async () => {
    const token = await signAccessToken("user-1", "alice@example.com");
  ...
  it("rejects a tampered access token", async () => {
    const token = await signAccessToken("user-3", "bob@example.com");
  ...
  it("issues a refresh expiry ~7 days out", () => {
