...
import { trainings, trainingFacilitators } from "../db/schema";
...
import { CreateTrainingSchema, UpdateTrainingSchema, AssignFacilitatorSchema } from "@oruclass/validators";
...
trainingsRouter.post(
  ...
  async (c) => {
    const { id: trainingId } = c.req.param();
    const body = await c.req.json<{ email: string; role: "trainer" | "assistant" | "full_editor" }>();

    // For now, if the user does not exist in the system, we might return an error or stub it.
...
trainingsRouter.patch(
  ...
  async (c) => {
    const workspaceId = c.get("workspaceId") as string;
    const { id } = c.req.param();
    const { status } = await c.req.json<{ status: "draft" | "connecting" | "live" | "paused" | "completed" }>();

    // Enforce one active session per workspace
    if (["connecting", "live", "paused"].includes(status)) {
      const activeSession = await db.query.trainings.findFirst({
        where: and(
          eq(trainings.workspaceId, workspaceId),
          not(eq(trainings.id, id)),
          inArray(trainings.sessionStatus, ["connecting", "live", "paused"]),
        ),
      });
